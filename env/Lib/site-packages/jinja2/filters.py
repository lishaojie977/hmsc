...
def do_round(value, precision=0, method="models"):
    """Round the number to a given precision. The first
    parameter specifies the precision (default is ``0``), the
    second the rounding method:

    - ``'models'`` rounds either up or down
    - ``'ceil'`` always rounds up
    - ``'floor'`` always rounds down

    If you don't specify a method ``'models'`` is used.
    ...
    .. sourcecode:: jinja
        ...
        {{ 42.55|round|int }}
            -> 43
    """
    if method not in {"models", "ceil", "floor"}:
        raise FilterArgumentError("method must be models, ceil or floor")
    if method == "models":
